<script lang="ts">
	...
	$effect(() => {
		if (browser) {
			const storedRole = localStorage.getItem('tea-role');
			const storedParty = localStorage.getItem('tea-party');
			if (storedRole) {
	...
	$effect(() => {
		if (browser) {
			if (role) {
				localStorage.setItem('tea-role', JSON.stringify(role));
			}
			if (party) {
				localStorage.setItem('tea-party', JSON.stringify(party));
			}
		}
	});
...
<Cheatsheet 
	title={`TEA Cheatsheet - ${optionsString}`}
	...
	tabTags={{"P1: Living Liquid": ['p1'], "Intermission: Limit Cut": ['i1'], "P2: BJCC": ['p2'], "Intermission: Temporal Stasis": ['i2'], "P3: Alexander Prime": ['p3'], "P4: Perfect Alexander": ['p4']}}
	splitTimeline={false}
/>
...
<div class="container grow px-4 mx-auto mb-6">
	<div class="container">
        <div class="mb-6">
            <div class="preset-typo-display-1 mt-2 lg:mt-0 lg:-mb-5">The Epic of Alexander (Ultimate)</div>
            <div class="text-xl lg:text-3xl text-surface-400">TEA Patch 5.11</div>
        </div>
			...
			<div class="space-y-5 v-full dark">
				...
				<div>
					...
					<Segment classes="flex-wrap" name="stratName" value={stratName} onValueChange={onSelectStrat}>
						<Segment.Item value="elina" labelClasses="flex items-center"><span class="badge preset-filled-primary-500 px-2 mr-2">NA</span>Elinabin</Segment.Item>
					</Segment>
				</div>
				<div>
		...
		{#if stratName && role && party}
			...
			<div class="card preset-filled-surface-50-950 border-[1px] border-surface-200-800 p-4">
				...
				<StratView
					...
					spotlight={spotlight}
					alignment={alignment}
					tabTags={{"P1: Living Liquid": ['setup','p1'], "Intermission: Limit Cut": ['i1'], "P2: BJCC": ['p2'], "Intermission: Temporal Stasis": ['i2'], "P3: Alexander Prime": ['p3'], "P4: Perfect Alexander": ['p4']}}
				/>
			</div>
			{/if}
		{/if}
