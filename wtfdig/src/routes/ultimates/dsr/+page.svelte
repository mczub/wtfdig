...
<script lang="ts">
	...
	$effect(() => {
		if (browser) {
			const storedRole = localStorage.getItem('dsr-role');
			const storedParty = localStorage.getItem('dsr-party');
			if (storedRole) {
	...
	$effect(() => {
		if (browser) {
			if (role) {
				localStorage.setItem('dsr-role', JSON.stringify(role));
			}
			if (party) {
				localStorage.setItem('dsr-party', JSON.stringify(party));
			}
		}
	});
...
<div class="container grow px-4 mx-auto mb-6">
	<div class="container">
		<div class="mb-6">
			<div class="preset-typo-display-1 mt-2 lg:mt-0 lg:-mb-2">Dragonsong's Reprise (Ultimate)</div>
			<div class="text-xl lg:text-3xl text-surface-400">DSR</div>
		</div>
