<script lang="ts">
	...
	$effect(() => {
		if (browser) {
			const storedRole = localStorage.getItem('ex5-role');
			const storedParty = localStorage.getItem('ex5-party');
			if (storedRole) {
	...
	$effect(() => {
		if (browser) {
			if (role) {
				localStorage.setItem('ex5-role', JSON.stringify(role));
			}
			if (party) {
				localStorage.setItem('ex5-party', JSON.stringify(party));
			}
		}
	});
...
<Cheatsheet 
	title={`EX5 Cheatsheet - ${optionsString}`}
...
<div class="container grow px-4 mx-auto mb-6">
	<div class="container">
        <div class="mb-6">
            <div class="preset-typo-display-1 mt-2 lg:mt-0 lg:-mb-5">The Minstrel's Ballad: Necron's Embrace</div>
            <div class="text-xl lg:text-3xl text-surface-400">EX5 Patch 7.3</div>
        </div>
