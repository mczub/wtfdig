<script lang="ts">
	...
	$effect(() => {
		if (browser) {
			const storedRole = localStorage.getItem('ex6-role');
			const storedParty = localStorage.getItem('ex6-party');
			if (storedRole) {
	...
	$effect(() => {
		if (browser) {
			if (role) {
				localStorage.setItem('ex6-role', JSON.stringify(role));
			}
			if (party) {
				localStorage.setItem('ex6-party', JSON.stringify(party));
			}
		}
	});
	...
	function setStratsFromUrlHash(hash: string) {
		...
		if (stratArray.length === 1) {
			...
			stratState = {
				lc: stratArray[1] ? stratArray[1] : getStratMechs(stratArray[0]).lc,
			}
		}
	}
	...
	function getStratCode(stratName: string | undefined, stratState: any) {
		...
		return `${stratName}:${getStratOrEmptyString('lc')}`;
	}
...
<Cheatsheet 
	title={`EX6 Cheatsheet - ${optionsString}`}
...
<div class="container grow px-4 mx-auto mb-6">
	<div class="container">
        <div class="mb-6">
            <div class="preset-typo-display-1 mt-2 lg:mt-0 lg:-mb-5">The Minstrel's Ballad: Windward Wilds</div>
            <div class="text-xl lg:text-3xl text-surface-400">EX6 Patch 7.3</div>
        </div>
				...
				<div class="flex flex-row space-x-4 space-y-2 flex-wrap mb-2">
					<div class="flex flex-col">
						<div class="flex flex-row">
							<div class="text-lg mb-2">Limit Cut</div>
						</div>
						<Segment classes="flex-wrap" name="variant" value={stratState.lc} onValueChange={(e) => (setStratState('variant', e.value))}>
							<Segment.Item value="diamond" labelClasses="flex items-center">Diamond</Segment.Item>
						</Segment>
					</div>
				</div>
				{/if}
