...
<div class="flex flex-wrap min-w-full justify-between mb-8 card preset-filled-surface-50-950 border-[1px] border-surface-200-800 p-4">
	<div class="space-y-5 v-full dark">
		...
		<div>
			...
			<Segment classes="flex-wrap" name="stratName" value={stratName} onValueChange={(e) => onSelectStrat(e.value!)}>
					...
					<div class="flex flex-col">
						...
						>
							{#each toggle.options as option}
								<Segment.Item value={option.value} labelClasses="flex items-center">{option.label}</Segment.Item>
							{/each}
						</Segment>
					</div>
				{/each}
		...
		<div>
			...
			<Segment name="role" classes="flex-wrap" value={role} onValueChange={(e) => setRole(e.value as Role)}>
		...
		<div>
			...
			<Segment name="party" value={party?.toString()} onValueChange={(e) => setParty(parseInt(e.value!))}>
