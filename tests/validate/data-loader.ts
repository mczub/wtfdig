...
import { fruFightConfig, fruStrats } from '../../src/routes/ultimates/fru/data';
...
export const ALL_FIGHTS: FightEntry[] = [
  ...
  { label: 'FRU', config: fruFightConfig, strats: fruStrats }
];
...
export function* walkAllDescriptions(fights?: FightEntry[]): Generator<DescriptionEntry> {
      ...
      for (const phase of strat.strats) {
        ...
        if (phase.mechs) {
          if (Array.isArray(phase.mechs)) {
            yield* walkMechDescriptions(phase.mechs, phasePath, strat.stratName, fight.label);
          } else {
...
export function getToggleByKey(config: FightConfig, key: string): FightToggleConfig | undefined {
